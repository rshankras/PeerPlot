...
final class AppService {
    ...
    convenience init(database: CouchbaseLiteSwift.Database, endpoint: AppService.Endpoint?, identity: SecIdentity, ca: SecCertificate) {
        self.init(database: database, endpoint: endpoint, conflictResolver: nil, identity: identity, ca: ca)
    }

    convenience init(database: CouchbaseLiteSwift.Database, endpoint: AppService.Endpoint?, conflictResolver: ConflictResolverProtocol? = nil, identity: SecIdentity? = nil, ca: SecCertificate? = nil) {
    ...
    init(name: String, database: CouchbaseLiteSwift.Database, collections: [CouchbaseLiteSwift.Collection], endpoint: AppService.Endpoint? = nil, conflictResolver: ConflictResolverProtocol? = nil, identity: SecIdentity? = nil, ca: SecCertificate? = nil) {
        self.name = name
        ...
        networkMonitor.pathUpdateHandler = { [weak self] path in
            switch path.status {
            case .satisfied:
                LogManager.info("Network is available",category: .network)
                self?.restart()
            default:
                LogManager.info("Network is unavailable",category: .network)
                self?.pause()
            }
        }
    ...
    private func createListener() -> NWListener? {
        ...
        do {
            ...
            LogManager.error("Failed to create listener: \(error)",category: .network)
            return nil
        }
        ...
        listener.newConnectionHandler = { [weak self] connection in
            ...
            self.setupConnectionHandshake(connection: connection, isFromListener: true)
        }
        ...
        listener.stateUpdateHandler = { newState in
            switch newState {
            case .ready:
                LogManager.info("Listener is ready",category: .network)
            case .failed(let error):
                LogManager.error("Listener stopped with error: \(error)",category: .network)
            case .cancelled:
                LogManager.info("Listener stopped with state: cancelled",category: .network)
            default:
                break
            }
        }
    ...
    private func createBrowser() -> NWBrowser {
        ...
        browser.stateUpdateHandler = { newState in
            switch newState {
            case .ready:
                LogManager.info("Browser is ready",category: .network)
            case .failed(let error):
                LogManager.error("Browser stopped with error: \(error)",category: .network)
            case .cancelled:
                LogManager.info("Browser stopped with state: cancelled",category: .network)
            default:
                break
            }
        }
    ...
    // Add this method to AppService class
    private func setupConnectionHandshake(connection: NWConnection, isFromListener: Bool) {
        LogManager.info("New connection: \(connection) from \(isFromListener ? "listener" : "browser")",category: .network)
        ...
        connection.receive(minimumIncompleteLength: self.uuid.count, maximumLength: self.uuid.count) { [weak self] (data, _, _, _) in
            ...
            guard !self.connections.contains(where: { self.serviceNameFor($0.endpoint) == remoteUUID }) else {
                LogManager.info("Skipping connection: \(remoteUUID)",category: .network)
                self.cleanupConnection(connection)
                return
            }
            
            // 3. Exchange availability messages (different based on connection source)
            if isFromListener {
                // For listener connections (incoming)
                self.handleListenerAvailabilityExchange(connection: connection, remoteUUID: remoteUUID)
            } else {
                // For browser connections (outgoing)
                self.handleBrowserAvailabilityExchange(connection: connection, remoteUUID: remoteUUID)
            }
        }
    ...
    private func handleListenerAvailabilityExchange(connection: NWConnection, remoteUUID: String) {
        ...
        connection.receive(minimumIncompleteLength: ConnectionEvents.available.count, maximumLength: ConnectionEvents.available.count) { [weak self] (data, _, _, _) in
            ...
            self.setupConnectionStateHandler(connection, isFromListener: true)
        }
    }

    private func handleBrowserAvailabilityExchange(connection: NWConnection, remoteUUID: String) {
        ...
        connection.receive(minimumIncompleteLength: ConnectionEvents.available.count, maximumLength: ConnectionEvents.available.count) { [weak self] (data, _, _, _) in
            ...
            connection.receive(minimumIncompleteLength: ConnectionEvents.ready.count, maximumLength: ConnectionEvents.ready.count) { [weak self] (data, _, _, _) in
                ...
                // Handle connection state changes
                self.setupConnectionStateHandler(connection, isFromListener: false)
            }
        }
    }

    private func setupConnectionStateHandler(_ connection: NWConnection, isFromListener: Bool) {
        ...
        connection.stateUpdateHandler = { [weak self] newState in
            ...
            switch newState {
            case .ready:
                LogManager.info("Connection ready: \(connection)",category: .network)
            ...
            case .failed(let error):
                LogManager.error("Connection failed with error: \(error)",category: .network)
                self.cleanupConnection(connection)
            case .cancelled:
                LogManager.info("Connection cancelled: \(connection)",category: .network)
                self.cleanupConnection(connection)
            default:
                break
            }
    ...
    private class NMMessageEndpointConnection: CouchbaseLiteSwift.MessageEndpointConnection {
        ...
        private func receive() {
            connection.receive(minimumIncompleteLength: 1, maximumLength: 65536) { [weak self] (data, _, _, error) in
                if let error = error {
                    LogManager.error("Receive error: \(error)",category: .network)
                    ...
                    if let data = data {
                        LogManager.info("Received data: \(data)",category: .network)
                        let message = Message.fromData(data)
                        self?.replicatorConnection?.receive(message: message)
                    } else {
                        LogManager.info("Received data: nil",category: .network)
                    }
